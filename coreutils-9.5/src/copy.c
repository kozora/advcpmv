...
void send_webhook(void)
{
  if (webhook_url) {
    struct timeval now;
    gettimeofday(&now, NULL);
    double seconds = (now.tv_sec - g_oStartTime.tv_sec) + (now.tv_usec - g_oStartTime.tv_usec) / 1000000.0;
    char time_buf[256];
    format_time(time_buf, seconds, true);

    char size_buf[256];
    file_size_format(size_buf, g_iTotalWritten, 1);

    char cmd[4096];
    snprintf(cmd, sizeof(cmd), 
             "curl -s -o /dev/null -H \"Content-Type: application/json\" "
             "-d '{\"msgtype\": \"text\", \"text\": {\"content\": \"Task Finished: Processed %d files. Total Size: %s. Duration: %s.\"}}' "
             "\"%s\"", 
             g_iFilesCopied, size_buf, time_buf, webhook_url);
    /* We do not background it here to ensure it sends before exit, 
       or we could if we want to return immediately. 
       For a notification, waiting a split second is fine. 
       Removing & to be safe about process cleanup. */
    int ret = system(cmd);
    (void)ret; 
  }
}
  ...
  while (*namep != '\0')
    {
      ...
      if (x->exclude_ptr && excluded_file_name (x->exclude_ptr, src_name))
        {
          free (src_name);
