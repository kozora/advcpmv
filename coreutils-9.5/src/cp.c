...
static struct option const long_opts[] =
{
  ...
  {"webhook", required_argument, nullptr, WEBHOOK_OPTION},
...
cp_option_init (struct cp_options *x)
{
  ...
  x->dest_info = nullptr;
  x->src_info = nullptr;
  x->exclude_ptr = new_exclude ();
  x->filter_active = false;
}
  ...
  while ((c = getopt_long (argc, argv, "abdfgHilLnprst:uvxPRS:TZ",
         ...
         != -1)
    {
      switch (c)
        ...
        case EXCLUDE_OPTION:
          add_exclude (x.exclude_ptr, optarg, EXCLUDE_WILDCARDS);
          x.filter_active = true;
          break;

        case INCLUDE_OPTION:
          add_exclude (x.exclude_ptr, optarg, EXCLUDE_WILDCARDS | EXCLUDE_INCLUDE);
          x.filter_active = true;
          break;
